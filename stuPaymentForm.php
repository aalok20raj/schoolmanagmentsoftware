...
?>
<?php
    $selectRegiNumber = mysqli_query($conn, "SELECT * FROM paymentbillbooklate ORDER BY pBBl_id DESC LIMIT 1");
        if(mysqli_num_rows($selectRegiNumber)>0){
            if($row = mysqli_fetch_assoc($selectRegiNumber)){
                $regi = $row['pBBl_billNo'];
                $get_number = str_replace("PB", "", $regi);
...
?>
        ...
        <title>Students Payment Form - Prinical Dashboard Students Success Convent</title>
<?php 
...
<div id="layoutSidenav_content">
    <?php 
        $selectStuList = mysqli_query($conn, "SELECT * FROM studentclasslist ");
        $fetchselectStu = mysqli_fetch_assoc($selectStuList);
    ?>
                <main>
                                ...
                                if (isset($_POST['g-recaptcha-response'])) {
                                    ...
                                    $response = json_decode($verifyResponse);
                                        if($response->success){
                                            ...
                                            $emailVerify = mysqli_query($conn, "SELECT * FROM paymentbillbooklate WHERE sl_emailAddress='$email'");
                                            ...
                                            if($count_verifyEmail == 1){
                                                ...
                                                $insertData = mysqli_query($conn, "INSERT INTO paymentbillbooklate(pBBl_billNo, pBBl_billNo_date, sl_name, sl_fatherName, sl_motherName, sl_gradName, sl_dob, sl_gender, sl_category, sl_nationality, sl_caddress, sl_paddress, sl_contentNumber, sl_alternativeNumber, sl_emailAddress) VALUES('$studRegNo','$studRegDate','$name','$father','$mother','$gauradian','$dob','$gender','$category','$religion','$caddress','$paddress','$mobile','$amobile','$email')");
                                                    if($insertData){
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-floating mb-3">
                                      ...
                                      <label for="floatingRegi">Payment Bill No.</label>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-4">
                                    <div class="form-floating mb-3">
                                      ...
                                      <label for="floatingRegi">Payment Date</label>
                                    </div>
                                </div>
                            </div>
